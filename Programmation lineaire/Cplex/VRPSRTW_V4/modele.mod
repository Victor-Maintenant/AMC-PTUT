...
//
minimize (sum(i in N, j in N, k in B)(Dmoyen[i][j]*x[i][j][k]) + sum(i in N, j in N, k in B) (late[i][k]*x[i][j][k]) + sum(k in B) (t_plus[k] + t_moins[k]));
...
c6a:   //A voir si c6a et c6b sont vraiment utiles
...
c9: // rajoute beaucoup moins de temps calcul que c8 --> donne le même nombre de trajets aux brancardiers
	forall(k in B)
		sum(i in N:i>0, j in N:j!=0) x[i][j][k] == d[k]; 
...
c11:
	forall(i in N: i!=0, j in N:(j!=0 && j!=i), k in B)
	  	(Tdep[i][k] + D[i] + Dmoyen[i][j]) + (x[i][j][k]-1) * M <= Tdep[j][k];
	 	
c12:
	forall(i in N: i!=0, k in B)
	  	(Tdep[i][k] >= Trdv[i] - D[i]) && (Tdep[i][k] <= Trdv[i] + R[i] - D[i]);


c13: 
	forall(i in N, k in B)
	  	late[i][k] == (Tdep[i][k] + D[i]) - Trdv[i];


c14:
	forall(i in N, j in N:i!=j, k in B)
	  	Trdv[i] + late[i][k] <= Hfin_apm[k];

  	
c17:
	forall(i in N, k in B)
	  	(Trdv[i] + late[i][k] <= Hfin_mat[k]) || (Tdep[i][k] >= Hdeb_apm[k]);
...
int trdv2[r]; 		//liste des heures de dépar
