...
//
minimize (sum(i in N, j in N, k in B)(Dmoyen[i][j]*x[i][j][k]) + sum(i in N, k in B) (late[i][k]) + sum(k in B) (t_plus[k] + t_moins[k]));
...
c21: // Heure de rdv ne peut pas être avant que le brancardier ne commence à travailler
	forall(i in N:i!=0, j in N: (j!=0 && j!=i), k in B)
	  	Tdep[i][k]+D[i] >= Hdeb_mat[k]* x[i][j][k];
  	
c17:
	forall(i in N, k in B)
	  	((Tdep[i][k] <= Hfin_mat[k] - D[i]) && (Tdep[i][k]+D[i] <= Hfin_mat[k])) || ((Tdep[i][k] >= Hdeb_apm[k]) && (Tdep[i][k]+D[i] >= Hdeb_apm[k] + D[i]));
